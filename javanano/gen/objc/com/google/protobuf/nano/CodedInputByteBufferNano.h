...
#ifndef _ComGoogleProtobufNanoCodedInputByteBufferNano_H_
#define _ComGoogleProtobufNanoCodedInputByteBufferNano_H_
...
/**
 ...
 @throws InvalidProtocolBufferNanoException  <code>value</code> does not match the last tag.
 */
- (void)checkLastTagWasWithInt:(jint)value;

/**
 @brief Decode a ZigZag-encoded 32-bit value.
...
/**
 @brief Decode a ZigZag-encoded 64-bit value.
 ZigZag encodes signed integers into values that can be efficiently encoded with varint.  (Otherwise, negative values must be sign-extended to 64 bits to be varint encoded, thus always taking 10 bytes on the wire.)
...
/**
 @brief Retrieves a subset of data in the buffer.
 The returned array is not backed by the original buffer array.
...
/**
 @brief Returns true if the stream has reached the end of the input.
 This is the case if either the end of the underlying input source has been reached or if the stream has reached a limit created using #pushLimit(int) .
 */
- (jboolean)isAtEnd;
...
/**
 @brief Sets <code>currentLimit</code> to (current position) + <code>byteLimit</code> .
...
/**
 @brief Read an enum field value from the stream.
 Caller is responsible for converting the numeric value to an actual enum.
 */
- (jint)readEnum;
...
/**
 ...
 @throws InvalidProtocolBufferNanoException The end of the stream or the current limit was reached.
 */
- (jbyte)readRawByte;
...
/**
 ...
 Protocol message parsers use this to read tags, since a protocol message may legally end wherever a tag occurs, and zero is not a valid tag number.
 */
- (jint)readTag;
...
/**
 @brief Resets the current size counter to zero (see #setSizeLimit(int) ).
 */
- (void)resetSizeCounter;

/**
 @brief Rewind to previous position.
 Cannot go forward.
 */
- (void)rewindToPositionWithInt:(jint)position;

/**
 @brief Set the maximum message recursion depth.
 In order to prevent malicious messages from causing stack overflows, <code>CodedInputStream</code> limits how deeply messages may be nested.  The default limit is 64.
 @return the old limit.
 */
...
/**
 ...
 In order to prevent malicious messages from exhausting memory or causing integer overflows, <code>CodedInputStream</code> limits how large a message may be. The default limit is 64MB.  You should set this limit as small as you can without harming your app's functionality.  Note that size limits only apply when reading from an <code>InputStream</code> , not when constructed around a raw byte array. <p> If you want to read several messages from a single CodedInputStream, you could call #resetSizeCounter() after each one to avoid hitting the size limit.
 @return the old limit.
 */
...
/**
 ...
 @return <code>false</code> if the tag is an endgroup tag, in which case nothing is skipped.  Otherwise, returns <code>true</code> .
 */
- (jboolean)skipFieldWithInt:(jint)tag;

/**
 @brief Reads and discards an entire message.
 This will read either until EOF or until an endgroup tag, whichever comes first.
 */
- (void)skipMessage;
...
#endif // _ComGoogleProtobufNanoCodedInputByteBufferNano_H_
